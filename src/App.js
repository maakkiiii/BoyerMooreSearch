...
function App() {
    ...
    function buttonhandler(){
        let txt = textarea;
        let pat = input;
        let occurences = search(txt, pat);
    }

    return (
      <>
        <h1>Boyer Moore Algorythmus</h1>
        <div className={"felder"}>
          <div className={"feld"}>
            <h2>Suchfeld</h2>
            <input type={"text"} onChange={(event) => setInput(event.target.value)}></input>
              <br/>
              <button>Suche</button></div>
          <div className={"feld"}>
            <h2>Textfeld</h2>
            <textarea title={"text field"} onChange={(event) => {setTextArea(event.target.value)}}>
